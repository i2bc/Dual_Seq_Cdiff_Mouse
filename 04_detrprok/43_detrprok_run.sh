...
mkdir detrprok_res ; 
covaS=100 ; covsR=3 ; cov5u=5 ; for i in `cat cond_fwtd2.txt cond_fwtTYsampled.txt cond_pBaseSampled.txt cond_pWT.txt ` ; do 
   bash DETRPROK_2.1.3.sh -bed bam_bed/${i}_ppR.bed -gff ../00_initial_data/NC_009089.1_MaGe.gff -read_len 100 -features_list "CDS|ncRNA|riboswitch|RNase_P_RNA|rRNA|SRP_RNA|tmRNA|tRNA" -op_gap 60 -clust_gap 20 -RNA_gap 25 -RNA_merge 25 -sRNA_coverage ${covsR} -asRNA_coverage ${covaS} -5utr_coverage ${cov5u} -verbose true ;
